...
describe('Functional flows', () => {
  ...
  it('adds a task from Quick Add', async () => {
    const { getByTestId, findByText } = await renderWithProviders(<HomeScreen navigation={nav as any} />);
  ...
  it('filters Done tasks', async () => {
    const { getByTestId, findByText, queryByText } = await renderWithProviders(
      <HomeScreen navigation={nav as any} />
    );
    await findByText('Map the onboarding flow');
  ...
  it('expands and collapses a task accordion', async () => {
    const { getByTestId, queryByText, findByText } = await renderWithProviders(
      <HomeScreen navigation={nav as any} />
    );
    await findByText('Map the onboarding flow');
  ...
  it('changes theme mode and palette', async () => {
    const { getByTestId } = await renderWithProviders(<SettingsScreen navigation={nav as any} />);
  ...
  it('confirms PIN removal', async () => {
    ...
    const { getByTestId } = await renderWithProviders(<ConfirmPinScreen navigation={nav as any} />);
